...
(function () {
    ...
    class FinanceManagerReviewQueue {
        ...
        async batchFinalizeSelected() {
            ...
            const confirmed = await (window.varydianAppConfirm
                ? window.varydianAppConfirm(
                    'Finalize selected submissions',
                    `Finalize ${items.length} submission(s)? Each will be final-approved and its reporting period locked when applicable.`,
                    { confirmText: 'Finalize all', cancelText: 'Cancel' }
                )
