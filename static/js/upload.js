...
class UploadService {
    ...
    async uploadFile(file) {
        ...
        try {
            ...
            if (data.success) {
                ...
                // Perform balance check after successful upload
    ...
    displayBalanceCheck(balanceData) {
        ...
        totalDebits.textContent = `R ${balanceData.total_debits.toLocaleString('en-ZA', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;

        totalCredits.textContent = `R ${balanceData.total_credits.toLocaleString('en-ZA', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;

        balanceDifference.textContent = `R ${balanceData.balance_difference.toLocaleString('en-ZA', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
        ...
        if (balanceData.is_balanced) {
            ...
            if (statusText) statusText.textContent = `Not balanced: Difference R ${balanceData.balance_difference.toLocaleString('en-ZA', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
    ...
    showUnbalancedOptions(balanceData) {
        ...
        const difference = balanceData.balance_difference;
        ...
        optionsSection.innerHTML = `
            <div class="unbalanced-options-container">
                ...
                <p class="recommendation">${balanceData.recommendation}</p>
    ...
    async proceedWithWarning() {
        ...
        try {
            ...
            const data = await SADPMRUtils.safeFetch('/api/proceed-unbalanced', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
    ...
    displayResults(summary) {
        ...
        this.updateElement('totalAssets', SADPMRUtils.formatCurrency(summary.total_assets || 0));

        this.updateElement('totalLiabilities', SADPMRUtils.formatCurrency(summary.total_liabilities || 0));

        this.updateElement('netAssets', SADPMRUtils.formatCurrency(summary.net_assets || 0));

        this.updateElement('surplus', SADPMRUtils.formatCurrency(summary.surplus_deficit || 0));
    ...
    redirectToMappingReview(mappingData) {
        ...
        try {
            ...
            // Redirect to mapping review page with session ID
            window.location.href = `/mapping?session_id=${mappingData.session_id}&review=true`;
