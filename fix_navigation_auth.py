#!/usr/bin/env python3
"""
Fix navigation links to include auth token for GitHub Pages
"""
...
# Enhanced authentication script with navigation link fixing
NAVIGATION_FIX_SCRIPT = '''    <script>
        ...
        function checkAuthGitHubPages() {
            ...
            if (authToken) {
                try {
                    ...
                    // Fix navigation links to include auth token
                    fixNavigationLinks(authToken);
        ...
        function fixNavigationLinks(authToken) {
            // Fix all navigation links to include auth token
            const navLinks = document.querySelectorAll('.nav-menu a');
            navLinks.forEach(link => {
                const href = link.getAttribute('href');
                if (href && href !== 'login.html') {
                    // Add auth token to navigation links
                    link.setAttribute('href', href + '?auth=' + authToken);
                }
            });
            
            // Fix other links that might need auth
            ...
            console.log('Navigation links fixed with auth token');
        }
...
def fix_navigation_auth():
    """Fix navigation links to include auth token"""
    docs_dir = Path(__file__).parent / 'docs'
    
    print("Fixing Navigation Links for GitHub Pages Auth")
    print("=" * 50)
    ...
    for page_name in files_to_update:
        ...
        if page_path.exists():
            try:
                ...
                # Replace existing script with navigation fix script
                script_pattern = r'(\s*<script>.*?</script>)'
                
                if '<script>' in content:
                    # Replace existing script
                    content = re.sub(script_pattern, NAVIGATION_FIX_SCRIPT, content, flags=re.DOTALL)
    ...
    print("\nNavigation Fix Features:")
    print("- All navigation links include auth token")
    print("- Users stay logged in when clicking links")
    ...
    print("4. Click navigation links - should stay logged in")
    print("5. Different roles see different navigation options")
    
    return success_count == len(files_to_update)

if __name__ == '__main__':
    success = fix_navigation_auth()
