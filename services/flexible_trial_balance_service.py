"""
Flexible Trial Balance Upload Service
Handles any kind of trial balance format with automatic detection and mapping
"""
...
from models.trial_balance_models import (
    trial_balance_model, TrialBalanceSession, TrialBalanceColumn, 
    TrialBalanceDataRow, MappingRule, GRAPChartOfAccounts
)


class FlexibleTrialBalanceService:
    """Service for handling flexible trial balance uploads"""
    
    def __init__(self):
        self.model = trial_balance_model
    ...
    def process_upload(self, file_path: str, user_id: str, filename: str) -> Dict:
        """
        Process a trial balance file upload with automatic format detection
        ...
        """
        print(f"🔄 FlexibleTrialBalanceService.process_upload called")
    ...
    def _create_session(self, file_path: str, user_id: str, filename: str) -> TrialBalanceSession:
    ...
    def _detect_structure(self, df: pd.DataFrame) -> Dict:
        """Detect trial balance structure"""
        structure_info = {
            'file_type': 'unknown',
            'has_headers': False,
            'column_mapping': {},
    ...
    def _determine_file_type(self, structure_info: Dict) -> str:
        ...
        if structure_info['has_headers'] and structure_info['detected_patterns']['has_account_codes']:
            if structure_info['detected_patterns']['has_multi_period']:
                ...
                return 'standard_trial_balance'
            else:
                return 'simple_trial_balance'
    ...
    def _create_columns(self, session_id: str, columns: pd.Index, structure_info: Dict) -> List[TrialBalanceColumn]:
        ...
        for i, col in enumerate(columns):
            ...
            trial_column = TrialBalanceColumn(
            ...
            trial_columns.append(trial_column)
        
        # Save columns to database
        self.model.create_columns(trial_columns)
    ...
    def _process_data_rows(self, session_id: str, df: pd.DataFrame, columns: List[TrialBalanceColumn], structure_info: Dict) -> List[TrialBalanceDataRow]:
    ...
    def _perform_auto_mapping(self, session_id: str, data_rows: List[TrialBalanceDataRow]) -> Dict:
    ...
    def _calculate_match_confidence(self, row: TrialBalanceDataRow, grap_account: GRAPChartOfAccounts) -> float:
    ...
    def _update_session_results(self, session: TrialBalanceSession, df: pd.DataFrame, structure_info: Dict, data_rows: List[TrialBalanceDataRow]):
    ...
    def get_session_data(self, session_id: str) -> Dict:
        """Get session data including trial balance data for validation"""
        try:
            ...
            # Convert data rows to trial balance data format
            trial_balance_data = []
            for row in data_rows:
    ...
    def process_grap_mapping(self, session_id: str, user_id: str) -> Dict:
        ...
        try:
            ...
            if not trial_balance_data:
                return {'success': False, 'error': 'No trial balance data found'}
...
# Global instance
flexible_trial_balance_service = FlexibleTrialBalanceService()
