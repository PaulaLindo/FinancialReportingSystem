"""
Trial Balance Database Models
Flexible models for handling any kind of trial balance format
"""
...
@dataclass
class TrialBalanceSession:
    """Represents a trial balance upload session"""
...
@dataclass
class TrialBalanceColumn:
    """Represents a column in a trial balance file"""
    id: Optional[str] = None
    session_id: str = ""
    column_name: str = ""
...
@dataclass
class TrialBalanceDataRow:
    """Represents a single row of trial balance data"""
    id: Optional[str] = None
    session_id: str = ""
    row_index: int = 0
    raw_data: Dict = None
    ...
    def __post_init__(self):
        ...
        if not self.session_id or self.session_id == "":
            raise ValueError(f"TrialBalanceDataRow: session_id cannot be empty. Got: '{self.session_id}'")
...
class TrialBalanceModel:
    """Flexible trial balance database model"""
    ...
    def create_session(self, session: TrialBalanceSession) -> str:
        """Create a new trial balance session"""
        try:
            print(f"💾 TrialBalanceModel.create_session called")
            ...
            print(f"📋 Inserting session data into trial_balance_sessions table...")
            result = self.client.table('trial_balance_sessions').insert(session_data).execute()
            ...
            raise Exception(f"Failed to create trial balance session: {str(e)}")
    
    def get_session(self, session_id: str) -> Optional[TrialBalanceSession]:
        """Get a trial balance session by ID"""
        try:
            result = self.client.table('trial_balance_sessions').select('*').eq('id', session_id).execute()
            if result.data:
                data = result.data[0]
                return TrialBalanceSession(
                    id=data['id'],
            ...
            raise Exception(f"Error getting trial balance session: {str(e)}")
    ...
    def update_session_status(self, session_id: str, status: str, metadata: Dict = None) -> bool:
        ...
        try:
            ...
            result = self.client.table('trial_balance_sessions').update(update_data).eq('id', session_id).execute()
            return result.data is not None
        except Exception as e:
            raise Exception(f"Error updating session status: {str(e)}")
    
    def get_user_sessions(self, user_id: str, limit: int = 50) -> List[TrialBalanceSession]:
        """Get all sessions for a user"""
        try:
            result = self.client.table('trial_balance_sessions').select('*').eq('user_id', user_id).order('created_at', desc=True).limit(limit).execute()
            sessions = []
            for data in result.data:
                sessions.append(TrialBalanceSession(
                    id=data['id'],
                    user_id=data['user_id'],
                    filename=data['filename'],
                    original_filename=data.get('original_filename', ''),
    ...
    def get_sessions_by_status(self, status: str, limit: int = 50) -> List[TrialBalanceSession]:
        """Get sessions by status"""
        try:
            result = self.client.table('trial_balance_sessions').select('*').eq('status', status).order('created_at', desc=True).limit(limit).execute()
    ...
    def create_columns(self, columns: List[TrialBalanceColumn]) -> bool:
        """Create column definitions for a session"""
        try:
            data = [col.to_dict() for col in columns]
            result = self.client.table('trial_balance_columns').insert(data).execute()
            return result.data is not None
        except Exception as e:
            raise Exception(f"Error creating columns: {str(e)}")
    
    def get_session_columns(self, session_id: str) -> List[TrialBalanceColumn]:
        """Get all columns for a session"""
        try:
            result = self.client.table('trial_balance_columns').select('*').eq('session_id', session_id).order('column_index').execute()
            columns = []
            for data in result.data:
                columns.append(TrialBalanceColumn(
                    id=data['id'],
    ...
    def update_column_mapping(self, column_id: str, mapped_to: str, confidence: float = 0.0) -> bool:
        ...
        try:
            ...
            result = self.client.table('trial_balance_columns').update(update_data).eq('id', column_id).execute()
            return result.data is not None
        except Exception as e:
    ...
    def create_data_rows(self, rows: List[TrialBalanceDataRow]) -> bool:
        """Create trial balance data rows"""
        try:
            data = [row.to_dict() for row in rows]
            result = self.client.table('trial_balance_data').insert(data).execute()
            return result.data is not None
        except Exception as e:
            raise Exception(f"Error creating data rows: {str(e)}")
    
    def get_session_data(self, session_id: str, limit: int = 1000, offset: int = 0) -> List[TrialBalanceDataRow]:
        """Get trial balance data for a session"""
        try:
            result = self.client.table('trial_balance_data').select('*').eq('session_id', session_id).eq('is_active', True).order('row_index').range(offset, offset + limit - 1).execute()
            rows = []
            for data in result.data:
                rows.append(TrialBalanceDataRow(
                    id=data['id'],
                    session_id=data['session_id'],
    ...
    def update_data_row(self, row_id: str, updates: Dict) -> bool:
        ...
        try:
            ...
            result = self.client.table('trial_balance_data').update(updates).eq('id', row_id).execute()
            return result.data is not None
        except Exception as e:
    ...
    def batch_update_mapping(self, session_id: str, mappings: List[Dict]) -> bool:
        ...
        try:
            result = self.client.table('trial_balance_data').upsert(mappings).execute()
            return result.data is not None
        except Exception as e:
    ...
    def get_session_summary(self, session_id: str) -> Dict:
        """Get trial balance session summary using database function"""
        try:
            result = self.client.rpc('get_trial_balance_summary', {'p_session_id': session_id}).execute()
            if result.data:
...
# Global instance
trial_balance_model = TrialBalanceModel()
